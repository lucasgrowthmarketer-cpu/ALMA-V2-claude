...
import { getMachineBySlug, getBrandBySlug, machinePhotos } from '../data/machinesData';
...
const MachinePage = () => {
  ...
  const relatedMachines = brand ? brand.machines.filter(m => m.id !== machine.id).slice(0, 3) : [];

  // Obtenir les vraies photos depuis le fichier Excel
  const machinePhotoData = machinePhotos[machine.slug];
  const realPhotos = machinePhotoData?.photos || [];

  // Déterminer l'image basée sur la catégorie si pas de photo réelle
  const getMachineImage = () => {
    const category = machine.categorie.toLowerCase();
    const subcat = machine.sous_categorie?.toLowerCase() || '';
    
    if (category === 'usinage') {
      if (subcat.includes('tournage') || subcat.includes('tour')) {
        return images.usinage.cncLathe1;
      } else if (subcat.includes('fraisage')) {
        return images.usinage.millingMachine;
      } else if (subcat.includes('rectif')) {
        return images.usinage.grinding;
      }
      return images.usinage.cncLathe2;
    } else if (category === 'tolerie') {
      if (subcat.includes('pliage')) {
        return images.tolerie.pressBrake;
      } else if (subcat.includes('laser') || subcat.includes('decoupe')) {
        return images.tolerie.laserCutting1;
      }
      return images.tolerie.sheetMetal;
    } else if (category === 'chaudronnerie') {
      if (subcat.includes('soudage')) {
        return images.chaudronnerie.welding;
      }
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedMachines.map((relMachine) => {
                ...
                const relPhotoData = machinePhotos[relMachine.slug];
                const relImage = relPhotoData?.photos?.[0] || 
                                (relMachine.categorie === 'usinage' ? images.usinage.cncLathe2 : 
                                relMachine.categorie === 'tolerie' ? images.tolerie.pressBrake :
                                images.chaudronnerie.fabrication);
                return (
                  <Link key={relMachine.id} to={`/machine/${relMachine.slug}`}>
