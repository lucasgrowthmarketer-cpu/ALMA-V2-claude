...
const PartenairesPage = () => {
  ...
  const usinage = brandsSimple.filter(b => b.categorie === 'usinage').sort((a, b) => a.nom.localeCompare(b.nom));
  const tolerie = brandsSimple.filter(b => b.categorie === 'tolerie').sort((a, b) => a.nom.localeCompare(b.nom));
  const chaudronnerie = brandsSimple.filter(b => b.categorie === 'chaudronnerie').sort((a, b) => a.nom.localeCompare(b.nom));
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          ...
          {tolerie.length > 0 && (
            <div className="mb-16">
              <div className="flex items-center gap-4 mb-8">
                <h2 className="text-3xl font-bold text-gray-900">Tôlerie</h2>
                <span className="px-4 py-1 bg-[#ef6110] text-white rounded-full text-sm font-semibold">
                  {tolerie.length} marques
                </span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {tolerie.map((brand) => (
