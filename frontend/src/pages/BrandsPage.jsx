...
const BrandsPage = () => {
  ...
  const filteredBrands = brandsSimple.filter(brand => {
    ...
    const matchesCategory = selectedCategory === 'all' || brand.categorie === selectedCategory;
  ...
  const categories = [
    ...
    { id: 'usinage', name: 'Usinage', count: brandsSimple.filter(b => b.categorie === 'usinage').length },
    { id: 'tolerie', name: 'Tôlerie', count: brandsSimple.filter(b => b.categorie === 'tolerie').length },
    { id: 'chaudronnerie', name: 'Chaudronnerie', count: brandsSimple.filter(b => b.categorie === 'chaudronnerie').length }
  ];
                      ...
                      <CardContent className="relative h-full p-6 flex flex-col">
                        ...
                        <div className="mb-auto">
                          <span className={'inline-block px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider ' +
                            (brand.categorie === 'usinage' ? 'bg-blue-100 text-blue-700' :
                             brand.categorie === 'tolerie' ? 'bg-green-100 text-green-700' :
